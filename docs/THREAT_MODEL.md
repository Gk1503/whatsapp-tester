...
| `POST/GET/DELETE /api/schedules*` | Requires session + `manage_schedules` | PRIVILEGED / HIGH-RISK (schedules execute unattended later) |
...
- **Controls**: `express.json({ limit: '10mb' })`, `multer` file size limit (5MB) + extension/MIME allowlist, `zod` caps on recipients (500)/groups (200)/bulk rows (5000)/message length (4096)/delay (300s), `lib/spreadsheet.js` enforces row/column/cell-length caps independent of the multer limit, tiered rate limiting per endpoint risk.
- **Residual risk**: no global request concurrency cap, no circuit breaker on the transport layer, no protection against a single very slow client holding a connection open (mitigated somewhat by Express/Node defaults, not hardened further this session). Load/soak/burst testing beyond the one baseline benchmark run is a ROADMAP item.
...
| Duplicate/replay send (double-click, retried request) | **Not yet mitigated** — no idempotency key. A double-submitted `/api/send` will send twice. Tracked in ROADMAP. |
| Scheduler double-fire across a process restart | Mitigated by design: `runTickIfReady()` only fires `status === 'scheduled'` entries and immediately flips them to `'sending'` before the async send loop starts, so a restart mid-send would leave the entry as `'sending'` rather than re-queuing it as `'scheduled'` — **residual risk**: a crash during the `'sending'` phase currently leaves that schedule stuck (no automatic recovery/requeue), which is safer (fails closed against duplicate sends) but requires manual intervention. |
